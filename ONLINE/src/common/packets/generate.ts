...
const getLastStructure = () =>
  packet.structures[packet.structures.length - 1];

const getLastEnum = () =>
  packet.enums[packet.enums.length - 1];

const getLastField = () =>
  packet.fields[packet.fields.length - 1];

const getLastFieldOfStructure = () =>
  getLastStructure().fields[getLastStructure().fields.length - 1];

const getLastValueOfEnum = () =>
  getLastEnum().values[getLastEnum().values.length - 1];

function parseAsText<T extends any>(
  obj: T,
  fieldName: keyof T
): Required<
  Pick<HTMLRewriterTypes.HTMLRewriterElementContentHandlers, 'text'>
> {
  return {
    text(text) {
...
function parseAsNumber<T extends any>(
  obj: T,
  fieldName: keyof T,
  hex?: boolean
): Required<
...
> {
  return {
    text(text) {
      ...
      if (text.lastInTextNode) {
        obj[fieldName] = parseInt(
          (hex ? "0x" : "") + (obj[fieldName] as any).trim()
        ) as any;
      }
    },
  };
...
const registerPacketTextField = (name: keyof Packet) =>
  rewriter.on("packet > " + name.toLowerCase(), {
    text(text) {
      parseAsText(packet, name).text(text);
    },
  });

const registerPacketNumberField = (
  name: keyof Packet,
  hex?: boolean
) =>
  rewriter.on("packet > " + name.toLowerCase(), {
...
function getReaderStringForType(
  type: PacketFieldType,
  index: string,
  ...
  switch (type) {
    ...
    case "Byte": {
      if (length != null) {
        return `GetByteValue(${bufferVarName}.getUint8(${index}), ${length}, ${leftShifted})`;
      }
    ...
    case "ShortBigEndian":
      return `${bufferVarName}.getUint16(${index}, false)`;

    case "ShortLittleEndian":
      return `${bufferVarName}.getUint16(${index}, true)`;

    case "IntegerBigEndian":
      return `${bufferVarName}.getUint32(${index}, false)`;

    case "IntegerLittleEndian":
      return `${bufferVarName}.getUint32(${index}, true)`;

    case "LongBigEndian":
      return `${bufferVarName}.getBigUint64(${index}, false)`;
    ...
    case "string":
    case "String":
      return `this._readString(${index}, ${index} + ${length ?? 0})`;

    case "Binary":
      return `this._readDataView(${index}, ${index} + ${
        length != null ? Number(length) : 0
      })`;

    case "Enum":
      return `GetByteValue(${bufferVarName}.getUint8(${index}), ${
        length ?? 8
      }, ${leftShifted})`;

    case "Structure[]":
      throw new Error(
        `Structure[] must be handled separately: ${index}`
      );

    default:
      return `throw new Error(\`not implemented for type: ${type}\`)`;
  }
}

function getTypeSizeInBytes(
  type: PacketFieldType,
  count?: number
): number {
  switch (type) {
    ...
    default:
      return `throw new Error(\`getTypeSizeInBytes not implemented for type: ${type}\`)` as any;
  }
}
  ...
  if (lastField) {
    if (lastField.type === "Structure[]") {
      ...
      if (lastField.itemCountField) {
        const innerS =
          p.structures.find(
            (s) => s.name === lastField.typeName
          ) ??
          globalStructures.find(
            (s) => s.name === lastField.typeName
          );

        if (!innerS) {
          return `throw new Error("Structure not found: ${lastField.typeName}")`;
        }

        return `${prevSize} + ${lastField.itemCountField} * ${getSizeOfStructure(
          p,
          innerS
        )}`;
      }
...
function generateArrayOfStructuresReaderCode(
  p: Packet,
  fieldName: string,
  index: number,
  s: Structure,
  fieldIndexerName: string,
  countField?: string
): string {
  const structureSize = s.length;
  const estSize = getSizeOfStructure(p, s);

  return `const ${fieldName}_count = ${
  ...
  .map(
    ({
      ...
      if (type === 'Structure[]') {
        ...
        return generateArrayOfStructuresReaderCode(
          p,
          name,
          index,
          struct,
          `${fieldName}_StartOffset`,
          itemCountField
        );
      }

      const bytesInField =
        getTypeSizeInBytes(type, length);

      return `      const ${name} = ${getReaderStringForType(
        type,
        `${fieldName}_StartOffset + ${index}`,
        'b',
        leftShifted ?? 0,
        bytesInField,
        length
...
${s.fields
  ...
  .join(',\n')}
      ...
      ${structureSize != null
        ? `${fieldName}_StartOffset += ${estSize};`
        : `${fieldName}_StartOffset += ${estSize};`}
    }`;
}
...
function getEnumName(
  ...
  if (PREDEFINED_ENUMS.has(typeName)) {
    return typeName;
  }

  const pEnum = p.enums.find(
    (e) => e.name === typeName
  );

  if (pEnum) {
    return `${p.name}${typeName}Enum`;
  }

  const globalEnum = globalEnums.find(
    (e) => e.name === typeName
  );

  if (globalEnum) {
    return globalEnum.name + 'Enum';
  }
...
function generateStructureDefinition(
  ...
  s.fields.forEach(({ type, name, typeName }) => {
    if (type === 'Enum') {
      ...
      if (!structure) {
        throw new Error(
          `Structure not found: ${typeName}`
        );
      }

      type = `${generateStructureDefinition(
        p,
        structure
      )}[]` as PacketFieldType;
    } else if (type === 'String') {
...
rewriter.on("packetDefinitions", {
  element(el) {
    el.onEndTag(() => {
      const any = packets.find((p) =>
        p.fields.some((f) => f.index == null)
      );
...
`);
      ...
      packets.forEach((packet) => {
        ...
        packet.enums.forEach((packetEnum) => {
          writer.write(`export enum ${getEnumName(
            packet,
            packetEnum.name
          )} {
    ${packetEnum.values
...
`
        ...
        if (packet.code != null) {
          writer.write(
            `  static readonly Code = 0x${byteToString(
              packet.code
            )};
`
          );
        ...
        if (hasSubCode) {
          writer.write(
            `  static readonly SubCode = 0x${byteToString(
              packet.subCode
            )};
`
          );
  ...
  static getRequiredSize(dataSize:number){
    return ${packet.name}Packet.DataOffset + 1${
          hasSubCode ? ` + 1` : ''
        } + dataSize;
  }
`);
...
`);
  ...
  ${
    hasSubCode
      ? `b.setUint8(${packet.name}Packet.DataOffset + 1, ${packet.name}Packet.SubCode);`
      : ''
  }
    return this;
  }
...
`);
        ...
        writer.write(`
  static createPacket(requiredSize: number${
          packet.length != null
            ? ` = ${packet.length}`
            : ''
        }): ${packet.name}Packet {
...
`);
        ...
        packet.fields.forEach(
          ({
            ...
            let getMethod: keyof typeof DataView.prototype =
              "getUint8";

            let setMethod: keyof typeof DataView.prototype =
              "getUint8";
...
`);
                ...
                writer.write(`  set${name}(str: string, count = ${Number(
                  length
                )}){
    const from = ${index};
...
`);
                ...
                writer.write(`  set${name}(data: number[], count = ${Number(
                  length
                )}){
...
`);
  ...
  get${name}(count: number${
    ...
    ${generateArrayOfStructuresReaderCode(
      packet,
      name,
      arrayStartIndex,
      s,
...
const registerFieldHandler = (
  root: string,
  accessor: () => Field
) => {
  rewriter.on(
    root + " > fields field > name",
    {
      text(text) {
  ...
  rewriter.on(
    root + " > fields field > itemcountfield",
    {
      text(text) {
        parseAsText(
          accessor(),
...
rewriter.on(
  "packet structures structure fields",
  {
    element() {
...
rewriter.on(
  "packet structures structure > fields field",
  {
    element() {
...
rewriter.on(
  "packet > enums enum > values enumValue",
  {
    element() {
...
rewriter.on(
  "packet > enums enum > values enumValue > name",
  {
    text(text) {
      parseAsText(
        getLastValueOfEnum(),
...
rewriter.on(
  "packet > enums enum > values enumValue > description",
  {
    text(text) {
      parseAsText(
        getLastValueOfEnum(),
...
rewriter.on(
  "packet > enums enum > values enumValue > value",
  {
    text(text) {
      parseAsNumber(
        getLastValueOfEnum(),
...
const getLastGlobalStructure = () =>
  globalStructures[globalStructures.length - 1];

const getLastFieldOfGlobalStructure = () =>
  getLastGlobalStructure().fields[
    getLastGlobalStructure().fields.length - 1
  ];
...
rewriter.on("packetDefinitions > structures", {
  element(el) {
    ...
    el.onEndTag(() => {
      console.log(
        `global structures parsed: ${globalStructures.length}`
      );
    });
  },
});
...
rewriter.on(
  "packetDefinitions > structures structure",
  {
    element() {
...
rewriter.on(
  "packetDefinitions > structures structure fields",
  {
    element() {
      getLastGlobalStructure().fields = [];
    },
...
rewriter.on(
  "packetDefinitions > structures structure > fields field",
  {
    element() {
...
rewriter.on(
  "packetDefinitions > structures structure > name",
  {
    text(text) {
      parseAsText(
        getLastGlobalStructure(),
        'name'
      ).text(text);
    },
  }
...
rewriter.on(
  "packetDefinitions > structures structure > description",
  {
    text(text) {
      parseAsText(
        getLastGlobalStructure(),
...
rewriter.on(
  "packetDefinitions > structures structure > length",
  {
    text(text) {
      parseAsNumber(
        getLastGlobalStructure(),
...
registerFieldHandler(
  "packetDefinitions > structures structure",
  getLastFieldOfGlobalStructure
);

const getLastGlobalEnum = () =>
  globalEnums[globalEnums.length - 1];

const getLastValueOfGlobalEnum = () =>
  getLastGlobalEnum().values[
    getLastGlobalEnum().values.length - 1
  ];
...
rewriter.on(
  "packetDefinitions > enums > enum > values",
  {
    element() {
      getLastGlobalEnum().values = [];
    },
...
rewriter.on(
  "packetDefinitions > enums > enum > values enumValue",
  {
    element() {
...
rewriter.on(
  "packetDefinitions > enums > enum > name",
  {
    text(text) {
      parseAsText(
        getLastGlobalEnum(),
...
rewriter.on(
  "packetDefinitions > enums > enum > description",
  {
    text(text) {
      parseAsText(
        getLastGlobalEnum(),
...
rewriter.on(
  "packetDefinitions > enums > enum > values enumValue > name",
  {
    text(text) {
      parseAsText(
        getLastValueOfGlobalEnum(),
        'name'
      ).text(text);
    },
  }
...
rewriter.on(
  "packetDefinitions > enums > enum > values enumValue > description",
  {
    text(text) {
      parseAsText(
        getLastValueOfGlobalEnum(),
...
rewriter.on(
  "packetDefinitions > enums > enum > values enumValue > value",
  {
    text(text) {
      parseAsNumber(
        getLastValueOfGlobalEnum(),
